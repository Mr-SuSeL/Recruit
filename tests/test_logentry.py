import datetime
import pytest
from profil_logger.logger import LogEntry, LogLevel 

def test_log_entry_initialization():
    """Test that LogEntry can be initialized correctly."""
    now = datetime.datetime.now()
    entry = LogEntry(date=now, level="INFO", message="Test message") 
...
def test_log_entry_str_representation():
    ...
    entry = LogEntry(date=test_date, level="WARNING", message="Something happened.") 
...
def test_log_entry_repr_representation():
    ...
    test_date = datetime.datetime(2023, 1, 15, 10, 30, 0)
    entry = LogEntry(date=test_date, level="ERROR", message="Critical error!") 
    expected_repr = (
        f"LogEntry(date={test_date!r}, "
        f"level={'ERROR'!r}, message={'Critical error!'!r})" 
    )
...
def test_log_entry_to_dict():
    ...
    entry = LogEntry(date=test_date, level="INFO", message="Data point.") 
    expected_dict = {
